(function() {
  var VERSION, action, cli, error, errors, flatten, fs, initialResource, log, params, path, positionals, program, resource, resourceName, resources, server, showUsage, writer, _base, _i, _len, _ref, _ref2;
    ...
    switch (action) {
      ...
      case 'serve':
        ...
        server.serve(initialResource.server, function() {
          return flatten(resource().resources);
        });
        break;
      case 'write':
        if (positionals.length >= 1) resourceName = positionals.shift();
