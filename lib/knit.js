(function() {
  var VERSION, action, builder, cli, compilers, config, dir, error, errors, fs, loadConfig, loadResources, params, path, positionals, previewer, program, server, showUsage, _base, _i, _len, _ref, _ref2;
  ...
  if ((action != null) && (action !== 'serve' && action !== 'build')) {
    errors.push("" + action + " is not a valid command. See --help for details.");
  }
  if (params != null ? params.help : void 0) {
    action = 'help';
  }
  if (params != null ? params.version : void 0) {
    action = 'version';
  }
  if (errors.length > 0) {
    ...
    global.knit = params;
    if ((_ref2 = (_base = global.knit).action) == null) {
      _base.action = action;
    }
    dir = path.resolve((params != null ? params.dir : void 0) || '.');
    loadResources = function() {
      return compilers.knit('/', dir, dir, {});
    };
    switch (action) {
      case 'version':
        console.log("" + VERSION);
